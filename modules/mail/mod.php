<?php
class mailSwr extends moduleSwr {
	...
	public function addOptions($opts) {
		$opts[ $this->getCode() ] = array(
			'label' => __('Mail', SWR_LANG_CODE),
			'opts' => array(
				'mail_function_work' => array('label' => __('Mail function tested and work', SWR_LANG_CODE), 'desc' => ''),
				'notify_email' => array('label' => __('Notify Email', SWR_LANG_CODE), 'desc' => __('Email address used for all email notifications from plugin', SWR_LANG_CODE), 'html' => 'text', 'def' => get_option('admin_email')),
			),
		);
		return $opts;
	}
