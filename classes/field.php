...
class fieldSwr {
	 ...
	 */
	public $adapt = array('htmlSwr' => '', 'dbFrom' => '', 'dbTo' => '');
	/**
	...
	public function setValue($value, $fromDB = false) {
		if(isset($this->adapt['dbFrom']) && $this->adapt['dbFrom'] && $fromDB)
			$value = fieldAdapterSwr::_($value, $this->adapt['dbFrom'], fieldAdapterSwr::DB);
		$this->value = $value;
	}
	...
	public function drawHtml($tag, $id) {
		if(method_exists('htmlSwr', $this->html)) {
			$method = $this->html;
			...
			if($this->adapt['html']) {
				fieldAdapterSwr::_($this, $this->adapt['html'], fieldAdapterSwr::HTML);
			}
			$params = $this->processParams($tag, $id);
			if ($params != '')
				return $params;
			if ($this->name == 'default_value') {
				$optionsFromDb = frameSwr::_()->getModule('optionsSwr')->getHelper()->getOptions($id);
				if (!empty($optionsFromDb)) {
					$options = array(0 => __('Select', SWR_LANG_CODE));
					foreach($optionsFromDb as $k => $v)
						$options[$k] = $v;
					$method = 'selectbox';
					$this->htmlParams['optionsSwr'] = $options;
				}
			}
			$htmlContent = htmlSwr::$method($this->name, $this->htmlParams);
			if(!empty($this->errorEl)) {
				if(is_bool($this->errorEl))
					$errorEl = '<div class="toeErrorForField toe_'. htmlSwr::nameToClassId($this->name). '"></div>';
	...
	public function displayValue() {
		...
		switch($this->html) {
			case 'countryList':
				$value = fieldAdapterSwr::displayCountry($this->value);
				break;
			case 'statesList':
				$value = fieldAdapterSwr::displayState($this->value);
				break;
			case 'checkboxlist':
				$options = $this->getHtmlParam('optionsSwr');
				$value = array();
	...
	public function viewField($name, $fieldValue = '') {
		$method = $this->html;
		$options = frameSwr::_()->getModule('optionsSwr')->getHelper()->getOptions($this->id);
		$attrs = '';
		...
		$params = array('optionsSwr'=>$options, 'attrs' => $attrs, 'value' => $fieldValue, 'checked' => $checked);
		$output = '';
		if(method_exists('htmlSwr', $method)) {
			$output .= htmlSwr::$method($name, $params);
			$output .= htmlSwr::hidden('extra_field['.$this->name.']',array('value'=>$this->id));
		}
		return $output;
	}
	...
	public function processParams($tag, $id){
		...
		if ($this->name == "params") {
			...
			switch ($tag) {
				case 5: 
					$add_option = __('Add Checkbox', SWR_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 9: 
					$add_option = __('Add Item', SWR_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 12:
				...
				case 10:
					$options_tag = '';
					$add_option = __('Add Radio Button', SWR_LANG_CODE);
					$image_tag = ' style="display:none"';
				break;
				case 8:
					$image_tag = '';
			...
			if ($tag > 0 || $id == 0) {
				$output .= '<div class="options options_tag"'.$options_tag.'>';
					...
					$output .= fieldAdapterSwr::_($id,'getExtraFieldOptions',fieldAdapterSwr::STR);
				$output .= '</div>';

				$output .= '<div class="options image_tag"'.$image_tag.'>'.__('Dimensions', SWR_LANG_CODE).':<br />';
					$params->width?$width = $params->width:'';
					$params->height?$height = $params->height:'';
					$output .= __('width', SWR_LANG_CODE).':<br />';
					$output .= htmlSwr::text('params[width]',array('value'=>$width)).'<br />';
					$output .= __('height', SWR_LANG_CODE).':<br />';
					$output .= htmlSwr::text('params[height]',array('value'=>$height)).'<br />';
				$output .= '</div>';
			}
			if($this->adapt['htmlParams']) {
				$output .= fieldAdapterSwr::_($this, $this->adapt['htmlParams'], fieldAdapterSwr::STR);
			} else {
				$output .= '<a href="javascript:void(0);" class="set_properties">'.__('Click to set field "id" and "class"', SWR_LANG_CODE).'</a>';
				$output .= '<div class="attributes" style="display:none;">'.__('Attributes', SWR_LANG_CODE).':<br />';
				$output .= fieldAdapterSwr::_($params,'getFieldAttributes',  fieldAdapterSwr::STR);
				$output .= '</div>';
			}
			return $output;
		}
	...
	function checkVarFromParam($param, $element) {
		return utilsSwr::xmlAttrToStr($param, $element);
	  ...
	  // load xml structure of parameters
	   ...
	   foreach ($config->params->param as $param) {
		 // read the variables
		  ...
		  $helper = $this->checkVarFromParam($param,'helperSwr');
		  $module = $this->checkVarFromParam($param,'moduleSwr');
	...
	public function drawConfig($module, $additionalOptions = array()) {
		if(!frameSwr::_()->getModule($module)) 
			return false; 
		// check for xml file with params structure  
	   if(frameSwr::_()->getModule($module)->isExternal())
		   $config_xml = frameSwr::_()->getModule($module)->getModDir(). 'mod.xml';
	   else
		   $config_xml = SWR_MODULES_DIR.$module.DS.'mod.xml';

	   if (!file_exists($config_xml)) {
		   // if there is no configuration file for this $module
		   return __('There are no configuration options for this module', SWR_LANG_CODE);
	   }
	   $output = '';
		   ...
		   foreach ($params as $param) {
			   ...
			   foreach ($configOptions as $key=>$value){
				  ...
				  // if helper is needed to render the object
					  ...
					  $helper = frameSwr::_()->getModule($hmodule)->getHelper();
					  if ($helper) {
				  ...
				  // filling the parameters to build html element
					 $htmlParams = array('value'=>$fieldValue,'optionsSwr'=>$options);
					 if($method == 'checkbox') {
						 $htmlParams['value'] = 1;
