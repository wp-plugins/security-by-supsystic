...
class utilsSwr {
    ...
    static public function copyDirectories($source, $destination) {
        if(is_dir($source)) {
            ...
            while ( FALSE !== ( $readdirectory = $directory->read() ) ) {
                ...
                if (is_dir($PathDir)) {
                    utilsSwr::copyDirectories( $PathDir, $destination . '/' . $readdirectory );
                    continue;
                }
    ...
    static public function xmlNodeAttrsToArr($node) {
        ...
        foreach($node->attributes() as $a => $b) {
            $arr[$a] = utilsSwr::xmlAttrToStr($node, $a);
        }
        return $arr;
    }
    ...
    static public function getCurrentWPThemeDir() {
        static $themePath;
        if(empty($themePath)) {
            $themePath = get_theme_root(). DS. utilsSwr::getCurrentWPThemeCode(). DS; 
        }
        return $themePath;
    }
    /**
    ...
    static public function getExtModPath($plugName) {
        if(strpos($plugName, 'wp-content/themes') !== false) {  //for modules in theme dir
            return SWR_SITE_URL. str_replace(DS, '/', $plugName). '/';
        } else
    ...
    static public function isThisCommercialEdition() {
        ...
        foreach($commercialModules as $m) {
            if(!frameSwr::_()->getModule($m)) 
                return false;
            if(!is_dir(frameSwr::_()->getModule($m)->getModDir())) 
                return false;
        }
        return true;*/
        foreach(frameSwr::_()->getModules() as $m) {
	...
	static public function isPluginsPage() {
		return (basename(reqSwr::getVar('SCRIPT_NAME', 'server')) === 'plugins.php');
	}
	...
	/**
	 * Parse worswress post/page/custom post type content for images and return it's IDs if there are images
