...
class fieldAdapterSwr {
    ...
    static public function _($fieldOrValue, $method, $type) {
        if(method_exists('fieldAdapterSwr', $method)) {
            switch($type) {
                case self::DB:
    ...
    static public function userFieldDestHtml($field) {
        $field->htmlParams['optionsSwr'] = array();
    ...
    static public function userFieldDestToDB($value) {
        return utilsSwr::jsonEncode($value);
    }
    static public function userFieldDestFromDB($value) {
        return utilsSwr::jsonDecode($value);
    }
    static public function taxDataHtml($field) {
        ...
        $categories = frameSwr::_()->getModule('products')->getCategories();
        $brands = frameSwr::_()->getModule('products')->getBrands();
        $cOptions = array();
        $bOptions = array();
        if(!empty($categories)) {
        ...
        return '<div>'. __('Apply To', SWR_LANG_CODE). '
            <div id="tax_address">
                <b>'. __('Address', SWR_LANG_CODE). '</b><br />
                '. __('Destination', SWR_LANG_CODE). ':'. htmlSwr::checkboxlist('params[dest]', array('optionsSwr' => $listOfDest)). '<br />
                '. __('Country', SWR_LANG_CODE). ':'. htmlSwr::countryList('params[country]', array('notSelected' => true, 'value' => $field->value['country'])). '<br />
            </div>
            <div id="tax_category">
                <b>'. __('Categories', SWR_LANG_CODE). '</b><br />
                '. (empty($cOptions) ? __('You have no categories', SWR_LANG_CODE) : htmlSwr::checkboxlist('params[categories][]', array('optionsSwr' => $cOptions))). '<br />
                    <b>'. __('Brands', SWR_LANG_CODE). '</b><br />
                '. (empty($bOptions) ? __('You have no brands', SWR_LANG_CODE) : htmlSwr::checkboxlist('params[brands][]', array('optionsSwr' => $bOptions))). '<br />
            </div>
            <div>'. __('Tax Rate', SWR_LANG_CODE).': '. htmlSwr::text('params[rate]', array('value' => $field->value['rate'])).'</div>
            <div>'. __('Absolute', SWR_LANG_CODE).': '. htmlSwr::checkbox('params[absolute]', array('checked' => $field->value['absolute'])).'</div>
        </div>';
    }
    ...
    static public function getExtraFieldOptions($field_id) {
        ...
        $options = frameSwr::_()->getModule('optionsSwr')->getHelper()->getOptions($field_id);
        if (!empty($options)) {
    ...
    static public function getFieldAttributes($params){
        ...
        if (!empty($params->attr)) {
            foreach ($params->attr as $key=>$value) {
                ...
                $output .= __('class', SWR_LANG_CODE).':<br />';
                $output .= htmlSwr::text('params[attr][class]',array('value'=>'')).'<br />';
                $output .= __('id', SWR_LANG_CODE).':<br />';
                $output .= htmlSwr::text('params[attr][id]',array('value'=>'')).'<br />';
        }
        return $output;
    }
    /**
    ...
    static function productFieldCategories($field){
        if(!empty($field->htmlParams['optionsSwr']))
            return;
	...
	static public function getCachedCountries($clearCache = false) {
		if(empty(self::$countries) || $clearCache)
			self::$countries = frameSwr::_()->getTable('countries')->getAll('id, name, iso_code_2, iso_code_3');
	...
	static public function getCachedStates($clearCache = false) {
		if(empty(self::$states) || $clearCache)
			self::$states = frameSwr::_()->getTable('states')
				->leftJoin( frameSwr::_()->getTable('countries'), 'country_id' )
