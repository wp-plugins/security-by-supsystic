...
class langSwr {
	...
	static public function extract($d = array('dir' => '', 'langSwr' => '')) {
		$data = array();
		if(isset($d['dir']) && !empty($d['dir']))
			$langDirPath = $d['dir'];
		else if(isset($d['langSwr']) && !empty($d['langSwr']))
			$langDirPath = SWR_LANG_DIR. $d['langSwr']. DS;
		else
			$langDirPath = SWR_LANG_DIR. SWR_WPLANG. DS;
