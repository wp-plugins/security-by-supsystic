...
abstract class controllerSwr {
	...
	protected function _createModel($name = '') {
		if(empty($name)) $name = $this->getCode();
		$parentModule = frameSwr::_()->getModule( $this->getCode() );
		$className = '';
		if(importSwr($parentModule->getModDir(). 'models'. DS. $name. '.php')) {
			$className = toeGetClassNameSwr($name. 'Model');
		}
	...
	protected function _createView($name = '') {
		...
		if(importSwr($parentModule->getModDir(). 'views'. DS. $name. '.php')) {
			$className = toeGetClassNameSwr($name. 'View');
		}
	 ...
	 * @example :
	 return array(
			S_METHODS => array(
				'save' => array(SWR_ADMIN),
				'remove' => array(SWR_ADMIN),
				'restore' => SWR_ADMIN,
			),
			S_USERLEVELS => array(
	 ...
	 * Can be used on of sub-array - SWR_METHODS or SWR_USERLEVELS
	 */
	public function getPermissions() {
		return array();
	}
	public function getModule() {
		return frameSwr::_()->getModule( $this->getCode() );
	}
	...
	public function getListForTbl() {
		...
		$page = (int) reqSwr::getVar('page');
		$rowsLimit = (int) reqSwr::getVar('rows');
		$orderBy = reqSwr::getVar('sidx');
		$sortOrder = reqSwr::getVar('sord');
		...
		// jqGrid search
		$isSearch = reqSwr::getVar('_search');
		if($isSearch) {
			$searchField = trim(reqSwr::getVar('searchField'));
			$searchString = trim(reqSwr::getVar('searchString'));
	...
	public function removeGroup() {
		$res = new responseSwr();
		if($this->getModel()->removeGroup(reqSwr::getVar('listIds', 'post'))) {
