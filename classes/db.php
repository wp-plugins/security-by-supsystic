...
class dbSwr {
     ...
     * #__  Worswress prefix
     * ^__  Store plugin tables prefix (@see SWR_DB_PREF if config.php)
    ...
    static public function prepareQuery($query) {
        ...
        return str_replace(
                array('#__', '^__', '@__'), 
                array($wpdb->prefix, SWR_DB_PREF, $wpdb->prefix. SWR_DB_PREF),
                $query);
    }
    ...
    static public function timeToDate($timestamp = 0) {
        if($timestamp) {
            if(!is_numeric($timestamp))
                $timestamp = dateToTimestampSwr($timestamp);
    ...
    static public function dateToTime($date) {
        ...
        if(strpos($date, SWR_DATE_DL)) return dateToTimestampSwr($date);
        $arr = explode('-', $date);
        return dateToTimestampSwr($arr[2]. SWR_DATE_DL. $arr[1]. SWR_DATE_DL. $arr[0]);
    }
