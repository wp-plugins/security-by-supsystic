<?php
abstract class viewSwr extends baseObjectSwr {
    /* 
     * @deprecated
     */
    protected $_tpl = SWR_DEFAULT;
    /*
	...
	public function getPath($tpl) {
		$path = '';
		$code = $this->_code;
		$parentModule = frameSwr::_()->getModule( $this->_code );
		$plTemplate = frameSwr::_()->getModule('options')->get('template');		// Current plugin template
		if(empty($plTemplate) || !frameSwr::_()->getModule($plTemplate))
			$plTemplate = '';
		if(file_exists(utilsSwr::getCurrentWPThemeDir(). 'swr'. DS. $code. DS. $tpl. '.php')) {
            $path = utilsSwr::getCurrentWPThemeDir(). 'swr'. DS. $code. DS. $tpl. '.php';
        } elseif($plTemplate && file_exists(frameSwr::_()->getModule($plTemplate)->getModDir(). 'templates'. DS. $code. DS. $tpl. '.php')) {
			$path = frameSwr::_()->getModule($plTemplate)->getModDir(). 'templates'. DS. $code. DS. $tpl. '.php';
	...
	public function getModel($code = '') {
		return frameSwr::_()->getModule( $this->_code )->getController()->getModel($code);
	}
