...
class frameSwr {
    ...
    public function __construct() {
        $this->_res = toeCreateObjSwr('response', array());
        
    }
    static public function getInstance() {
        static $instance;
        if(!$instance) {
            $instance = new frameSwr();
        }
        return $instance;
    }
        ...
        if($activeModules) {
            foreach($activeModules as $m) {
                ...
                if(!empty($m['ex_plug_dir'])) {
                    $moduleLocationDir = utilsSwr::getExtModDir( $m['ex_plug_dir'] );
                }
                if(is_dir($moduleLocationDir. $code)) {
                    $this->_allModules[$m['code']] = 1;
                    if((bool)$m['active']) {
                        importClassSwr($code. strFirstUp(SWR_CODE), $moduleLocationDir. $code. DS. 'mod.php');
                        $moduleClass = toeGetClassNameSwr($code);
    ...
    public function init() {
		...
		dispatcherSwr::doAction('afterModulesInit');
		
		modInstallerSwr::checkActivationMessages();
        ...
        register_activation_hook(  SWR_DIR. DS. SWR_MAIN_FILE, array('utilsSwr', 'activatePlugin')  ); //See classes/install.php file
        register_uninstall_hook(SWR_DIR. DS. SWR_MAIN_FILE, array('utilsSwr', 'deletePlugin'));
        
		add_action('init', array($this, 'connectLang'));
        //$operationTime = microtime(true) - $startTime;
    }
	public function connectLang() {
		load_plugin_textdomain(SWR_CODE, false, SWR_DIR. 'lang');
	}
    /**
    ...
    public function checkPermissions($code, $action) {
        if($this->havePermissions($code, $action))
            ...
            exit(_e('You have no permissions to view this page', SWR_LANG_CODE));
        }
    }
    /**
    ...
    public function havePermissions($code, $action) {
        ...
        if($mod) {
            ...
            if(!empty($permissions)) {  // Special permissions
                if(isset($permissions[SWR_METHODS]) 
                    && !empty($permissions[SWR_METHODS])
                ) {
                    foreach($permissions[SWR_METHODS] as $method => $permissions) {   // Make case-insensitive
                        $permissions[SWR_METHODS][strtolower($method)] = $permissions;
                    }
                    if(array_key_exists($action, $permissions[SWR_METHODS])) {        // Permission for this method exists
                        $currentUserPosition = frameSwr::_()->getModule('user')->getCurrentUserPosition();
                        if((is_array($permissions[ SWR_METHODS ][ $action ]) && !in_array($currentUserPosition, $permissions[ SWR_METHODS ][ $action ]))
                            || (!is_array($permissions[ SWR_METHODS ][ $action ]) && $permissions[SWR_METHODS][$action] != $currentUserPosition)
                        ) {
                            $res = false;
                        }
                    }
                }
                if(isset($permissions[SWR_USERLEVELS])
                    && !empty($permissions[SWR_USERLEVELS])
                ) {
                    $currentUserPosition = frameSwr::_()->getModule('user')->getCurrentUserPosition();
					// For multi-sites network admin role is undefined, let's do this here
					if(is_multisite() && is_admin() && is_super_admin()) {
						$currentUserPosition = SWR_ADMIN;
					}
                    foreach($permissions[SWR_USERLEVELS] as $userlevel => $methods) {
                        if(is_array($methods)) {
    ...
    protected function _extractTables($tablesDir = SWR_TABLES_DIR) {
    ...
    protected function _extractTable($tableName, $tablesDir = SWR_TABLES_DIR) {
        importClassSwr('noClassNameHere', $tablesDir. $tableName. '.php');
        $this->_tables[$tableName] = tableSwr::_($tableName);
    }
    /**
     ...
     * @return object table
     * @example frameSwr::_()->getTable('products')->getAll()
     */
    ...
    public function addScript($handle, $src = '', $deps = array(), $ver = false, $in_footer = false, $vars = array()) {
		$src = empty($src) ? $src : uriSwr::_($src);
    ...
    /**
     * Add all scripts from _scripts array to worswress
     */
    ...
    public function isTplEditor() {
        $tplEditor = reqSwr::getVar('tplEditor');
	...
	public function isAdminPlugOptsPage() {
		$page = reqSwr::getVar('page');
		if(is_admin() && strpos($page, frameSwr::_()->getModule('adminmenu')->getMainSlug()) !== false)
			return true;
		return false;
	}
