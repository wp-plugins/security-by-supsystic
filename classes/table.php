...
abstract class tableSwr {
    ...
    static public function getInstance($table = '') {
        ...
        if(!isset($instances[$table])) {
            $class = 'table'. strFirstUp($table). strFirstUp(SWR_CODE);
            ...
            if($id) {
                ...
                foreach($data as $field) {
                    ...
                    foreach($field as $k => $v) {
                        if(isset($this->_fields[$k])) {
                            $row[$k] = toeCreateObjSwr('fieldSwr', array(
                                    $this->_fields[$k]->name,
                                    $this->_fields[$k]->html,
                                    $this->_fields[$k]->type,
    ...
    public function getTable($transform = false) {
        if($transform)
            return dbSwr::prepareQuery($this->_table);
        else
            return $this->_table;
    }
    ...
    public function get($fields = '*', $where = '', $tables = '', $return = 'all') {
        ...
        return dbSwr::get($query, $return);
    }
    public function store($data, $method = 'INSERT', $where = '') {
		$this->_clearErrors();
        $method = strtoupper($method);
		if($this->_escape) {
			$data = dbSwr::escape($data);
		}
        $query = '';
        switch($method) {
            case 'INSERT':
        ...
        if(empty($fields)) {
            $this->_addError(__('Nothig to update', SWR_LANG_CODE));
            return false;
        }
        ...
        if(dbSwr::query($query)) {
            if($method == 'INSERT')
                return dbSwr::lastID();
            else
                return true;
        } else
			$this->_addError(__('Database error. Please contact your developer.', SWR_LANG_CODE));
        return false;
    }
                ...
                if(array_key_exists($k, $this->_fields) || $k == $this->_id) {
                    ...
                    if($validate) {
                        if (is_object($this->_fields[$k])) {
                            ...
                            if($errors = validatorSwr::_($objForValidation)) {
    ...
    /**
     * Add new fieldSwrSwr for children table (@see class field)
    ...
    protected function _addField($name, $html = 'text', $type = 'other', $default = '', $label = '', $maxlen = 0, $dbAdapt = '', $htmlAdapt = '', $description = '') {
        $this->_fields[$name] = toeCreateObjSwr('fieldSwr', array($name, $html, $type, $default, $label, $maxlen, $dbAdapt, $htmlAdapt, $description));
        return $this;
    }
    ...
    public function exists($value, $field = '') {
        if(!$field)
            $field = $this->_id;
        return dbSwr::get('SELECT '. $this->_id. ' FROM '. $this->_table. ' WHERE '. $field. ' = "'. $value. '"', 'one');
    }
            ...
            switch($f->type) {
                case 'date':
                    if($d[$key] == '0000-00-00' || empty($d[$key]))
                        ...
                        $d[$key] = date(SWR_DATE_FORMAT, dbSwr::dateToTime($d[$key]));
                    }
                    break;
                case 'int':
                case 'tinyint':
    ...
    public function getLastInsertID() {
        return dbSwr::get('SELECT MAX('. $this->_id. ') FROM '. $this->_table, 'one');
    }
