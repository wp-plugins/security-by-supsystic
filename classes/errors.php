...
class errorsSwr {
    ...
    static public function setSession($error) {
        ...
        if(is_array($error))
            ...
            $sesErrors[] = $error;
        reqSwr::setVar('sesErrors', $sesErrors, 'session');
    }
    static public function init() {
        $swrErrors = reqSwr::getVar('swrErrors');
        if(!empty($swrErrors)) {
            if(!is_array($swrErrors)) {
                $swrErrors = array( $swrErrors );
            }
            $swrErrors = array_map('htmlspecialchars', array_map('stripslashes', array_map('trim', $swrErrors)));
            if(!empty($swrErrors)) {
                self::$current = $swrErrors;
				if(is_admin()) {
					add_action('admin_notices', array('errorsSwr', 'showAdminErrors'));
				} else {
					add_filter('the_content', array('errorsSwr', 'appendErrorsContent'), 99999);
				}
            }
        }
    }
    ...
    static public function getSession() {
        return reqSwr::getVar('sesErrors', 'session');
    }
    static public function clearSession() {
        reqSwr::clearVar('sesErrors', 'session');
    }
