...
class htmlSwr {
    ...
    static public function img($src, $usePlugPath = 1, $params = array('width' => '', 'height' => '', 'attrs' => '')) {
        if($usePlugPath) $src = SWR_IMG_PATH. $src;
        return '<img src="'.$src.'" '
    ...
    static public function ajaxfile($name, $params = array('url' => '', 'value' => '', 'fid' => '', 'buttonName' => '')) {
		frameSwr::_()->addScript('ajaxupload', SWR_JS_PATH. 'ajaxupload.js');
        $out = '';
        if(strpos($params['url'], 'pl='. SWR_CODE) === false)
			$params['url'] = uriSwr::_(array('baseUrl' => $params['url'], 'pl' => SWR_CODE));
        $out .= self::button(array('value' => langSwr::_( empty($params['buttonName']) ? 'Upload' :  $params['buttonName'] ), 'attrs' => 'id="toeUploadbut_'.$name.'" class="button button-large"'));
    ...
    static public function statesInput($name, $params = array('value' => '', 'attrs' => '', 'notSelected' => true, 'id' => '', 'selectHtml' => '')) {
        ...
        $states = fieldAdapterSwr::getStates($params['notSelected']);
        ...
        if(empty($params['doNotAddJs'])) {
            $res .= '<script type="text/javascript">
                // <!--
                if(!toeStates.length)
                    toeStates = '. utilsSwr::jsonEncode($states). ';
    ...
    static public function countryList($name, $params = array('value' => '', 'attrs' => '', 'notSelected' => true)) {
        $params['notSelected'] = isset($params['notSelected']) ? $params['notSelected'] : true;
        $params['options'] = fieldAdapterSwr::getCountries($params['notSelected']);
    ...
    static public function textFieldsDynamicTable($name, $params = array('value' => '', 'attrs' => '', 'options' => array())) {
        ...
        if(!empty($params['options'])) {
            ...
            foreach($params['options'] as $key => $p) {
                $pattern[$key] = htmlSwr::text($name. '[]['. $key. ']');
            }
            ...
            elseif(!is_array($params['value'])) {
                $params['value'] = utilsSwr::jsonDecode($params['value']);
            ...
            do {
                ...
                foreach($params['options'] as $key => $p) {
                    ...
                    $res .= langSwr::_($p['label']). htmlSwr::text($name. '['. $i. ']['. $key. ']', $paramsForText);
                }
                $res .= $remove. '</div>';
                $i++;
    ...
    static protected function _loadProductsOptions() {
        if(empty(self::$productsOptions)) {
            $products = frameSwr::_()->getModule('products')->getModel()->get(array('getFields' => 'post.ID, post.post_title'));
            if(!empty($products)) {
	...
	static public function fontsList($name, $params = array('value' => '')) {
		...
		if(empty($options)) {	// Fill them only one time per loading
			foreach(fieldAdapterSwr::getFontsList() as $font)
				$options[ $font ] = $font;
		}
	...
	static public function slideInput($name, $params = array('attrs' => '', 'checked' => false, 'id' => '')) {
		...
		return '<a class="toeSlideShellSwr" href="#"'. $params['attrs']. '>
			<span class="toeSlideButtSwr"></span>
			<span class="toeSlideOnSwr">'. __('ON'). '</span>
			<span class="toeSlideOffSwr">'. __('OFF'). '</span>
	...
	static public function galleryBtn($name, $params = array()) {
		...
		$buttonParams = $params;
		$buttonParams['value'] = isset($params['btnVal']) ? $params['btnVal'] : sprintf(__('Select %s', SWR_LANG_CODE), strFirstUp($galleryType));
				...
				jQuery("#'. $buttonId. '").click(function(){
					...
					wp.media.editor.send.attachment = function(props, attachment){
						if ( _custom_media ) {
							...
							'. ($onChange ? $onChange. '(attachment.url, attachment);' : ''). '
						} else {
	...
	/*static public function radialProgress($progress, $attrs = array()) {
		frameSwr::_()->addStyle('radial-progress', SWR_CSS_PATH. 'radial-progress.css');
