 ...
 */
if(!function_exists('dateToTimestampSwr')) {
    function dateToTimestampSwr($date) {
        if(empty($a))
            return false;
        $a = explode(SWR_DATE_DL, $date);
...
if(!function_exists('importSwr')) {
    function importSwr($path) {
        if(file_exists($path)) {
            require($path);
            return true;
        }
...
if(!function_exists('importClassSwr')) {
    function importClassSwr($class, $path = '') {
        if(!class_exists($class)) {
            if(!$path) {
				$classFile = $class;
				if(strpos(strtolower($classFile), SWR_CODE) !== false) {
					$classFile = preg_replace('/'. SWR_CODE. '/i', '', $classFile);
				}
                $path = SWR_CLASSES_DIR. $classFile. '.php';
			}
            return importSwr($path);
            ...
            /*if(class_exists('ReflectionClass')) {   //ReflectionClass supported begining from php5
                ...
                if(strpos($classFile, SWR_DIR) === false) {   //Class is not in our plugin directory
                    ...
                    if(!empty($plugins)) {
                        for($i = 0; $i < count($plugins); $i++) {
                            if(strpos($plugins[$i], SWR_PLUG_NAME) !== false) {   //Let's remove our plugin from list of active plugins
                                unset($plugins[$i]);
                            }
                        }
 ...
 */
if(!function_exists('toeGetClassNameSwr')) {
    function toeGetClassNameSwr($class) {
        $className = '';
		if(class_exists($class. strFirstUp(SWR_CODE))) 
			$className = $class. strFirstUp(SWR_CODE);
		else if(class_exists(SWR_CLASS_PREFIX. $class)) 
            $className = SWR_CLASS_PREFIX. $class;
		else
            $className = $class;
        return $className;
    } 
 ...
 */
if(!function_exists('toeCreateObjSwr')) {
    function toeCreateObjSwr($class, $params) {
        $className = toeGetClassNameSwr($class);
        $obj = NULL;
 ...
 * @return array
 */
if(!function_exists('prepareParamsSwr')) {
	function prepareParamsSwr(&$d=array(), &$options = array()) {
		if (!empty($d['params'])) {
			...
			if (is_array($d['params'])) {
				$params = utilsSwr::jsonEncode($d['params']);
		...
		if(isset($d['code'])) {
			if ($d['code'] == '') {
				$d['code'] = prepareFieldCodeSwr($d['label']).'_'.rand(0, 9999999);
			}
		}
		return $d;
	}
}
if(!function_exists('prepareFieldCodeSwr')) {
	function prepareFieldCodeSwr($string) {   
